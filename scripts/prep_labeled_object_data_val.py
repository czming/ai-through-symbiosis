...
def get_list_colors_new():
    ...
    pick_label_path = 'data/pick_labels_gt/'
...
if __name__ == '__main__':
    ...
    for fil in test_picklists:
        ...
        htk_file = '../htk_outputs/icassp_test_folds/avgFold/results-' + pick_id + '.txt'
        ...
        if int(pick_id) not in picklist_label_dict.keys():
            ...
            # print(type(pick_id))
            continue
        if not os.path.exists('data/extracted_frames_test/'+fil_name):
            os.makedirs('data/extracted_frames_test/'+fil_name)
            ...
            if cur_time >= carry_times[time_idx] and cur_time <= carry_times[time_idx+1]:
                ...
                if hand is not None:
                    ...
                    if hand.shape[0]>0 and hand.shape[1]>0:
                        ...
                        cv2.imwrite('data/extracted_frames_test/'+fil_name+'/'+str(count)+'.png', hand)
    ...
    df['frame'] = frame_ids
    df['label'] = labels
    df.to_csv('data/labeled_objects_test.csv')
    ...
    df['frame'] = frame_ids
    df['label'] = labels
    df.to_csv('data/labeled_objects_val.csv')
