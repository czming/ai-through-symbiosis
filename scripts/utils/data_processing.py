...
def get_avg_hsv_bin_frames(hsv_inputs, start_frame, end_frame):
    ...
    for j in range(start_frame, end_frame):
        ...
        for k in range(len(hsv_bin_sum)):
            ...
            # k + 72 in both instances when looking at original main.py generated data
...
def avg_hsv_bins(hsv_inputs, elan_boundaries, action_label):
    ...
    if frame_count == 0:
        # no detected frames
        return np.array([0 for i in range(10)]).astype(float)
...
def vectors_rms_error(vector_arr1, vector_arr2):
    # calculate the rms error between arrays of vectors (rms error of the euclidean distance between vectors)
    sum_squared_error = 0

    if len(vector_arr1) != len(vector_arr2):
        raise Exception(f"vector_arr1 and vector_arr2 have different lengths, len(vector_arr1) = {len(vector_arr1)}" + \
                        f", len(vector_arr2) = {len(vector_arr2)}")
    elif len(vector_arr1) == 0:
        raise Exception("Empty arrays")

    for i in range(len(vector_arr1)):
        vector1 = np.array(vector_arr1[i])
        vector2 = np.array(vector_arr2[i])
        ...
        if len(vector1) == 0 or len(vector2) == 0:
            ...
            raise Exception(
                f"different length vectors, len(vector1) = {len(vector1)}" + \
                f", len(vector2) = {len(vector2)}")

        sum_squared_error += ((vector1 - vector2) ** 2).sum()

    return math.sqrt(sum_squared_error / len(vector_arr1))