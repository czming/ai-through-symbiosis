...
def avg_hsv_bins(hsv_inputs, htk_boundaries, action_label):
    ...
    for i in range(0, len(htk_boundaries[action_label]), 2):
        ...
        start_frame = math.floor(float(htk_boundaries[action_label][i]) * 59.97)
        end_frame = math.floor(float(htk_boundaries[action_label][i + 1]) * 59.97) - 10 # last frame was giving index out of bounds, and lossing 1 frame wont impact histogram
...
configs = load_yaml_config("C:/Users/chngz/Documents/AI through Symbiosis/HandGestureRecognition/scripts/configs/zm.yaml")
...
for picklist_no in PICKLISTS:
    ...
    # with open("./test.txt") as infile:
        htk_inputs = [i.split()[72:92] for i in infile.readlines()]

    carry_empty_bins = avg_hsv_bins(htk_inputs, elan_boundaries, "carry_empty")
    red_hsv_bins = avg_hsv_bins(htk_inputs, elan_boundaries, "carry_red") # red
    green_hsv_bins = avg_hsv_bins(htk_inputs, elan_boundaries, "carry_green") # green
    blue_hsv_bins = avg_hsv_bins(htk_inputs, elan_boundaries, "carry_blue") # blue
...
print(red_rmse + blue_rmse + green_rmse)
