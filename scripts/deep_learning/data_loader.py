...
class EgoObjectClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.test:
            image = read_image(data_folder+'/extracted_frames_test/'+self.picklist[idx]+'/'+str(self.frames[idx])+'.png')
        else:
            image = read_image(data_folder+'/extracted_frames/'+self.picklist[idx]+'/'+str(self.frames[idx])+'.png')
