...
###############################################################################
#
# this runs GT2K experiments.  Before this file is run, options.sh should 
# be populated with the appropriate values.  
#
# maintained by haileris@cc.gatech.edu, turtle@cc.gatech.edu
...
#If we're not doing auto-estimation, we can ignore the NUM_HMM_DIR
# paramter and leave it at 3. The model training process generates four
# iterations per HMM when we don't do auto-estimation. The final,
# trained model is stored in the third one.
if [[ "${AUTO_ESTIMATE}" == "no" ]]; then
	NUM_HMM_DIR = 3
fi
