...
UTIL_DIR=/usr/local/gt2k/utils			# location of utils directory
						#
						#
VECTOR_LENGTH=0					# number of elements in your
...
HMM_LOCATION=$HMM_TOPOLOGY_DIR/<none>		# text hmm topology file
HMM_ALL=$HMM_LOCATION				# unless otherwise specified
						# all gestures will be modeled
...
# Example of how to specify a different HMM topology for individual gestures.
#  
# HMM_TOKEN_1=$HMM_TOPOLOGY_DIR/5_state_2-4skip_loopback_3vec
# HMM_TOKEN_2=$HMM_TOPOLOGY_DIR/7state_noskip_3vec
# HMM_TOKEN_3=$HMM_TOPOLOGY_DIR/5state_noskip_3vec
#
# NOTE: using the above also requires that the corresponding section in
#       the "train.sh" also be uncommented (search for "$HMM_TOKEN_1").


# whether or not to initialize the starting model in a generic way:
# (We strongly recommend leaving this value set to "yes")
INITIALIZE_HMM=yes				# if you have informed values
						# for the initial model params,
						# say no here. In most cases
						# it is better to let HTK
...
DICTFILE=${PRJ}/dict				# dict is generated for you
...
EXT_DIR=${PRJ}/ext				# This is where HTK will put
						...
						#
GEN_GRAMMAR=yes					# yes/no: generate grammar and
						# dict files using specified
						# GRAMMAR_PROG program
...
${HTKBIN=}					# check to see if the path of
	#example:  ${HTKBIN=/usr/local/bin/}	# HTK is set as an environment
						...
						# trailing slash!
						#
PROMPT_B4_RM="no"				# UNTESTED if set to YES!!!
